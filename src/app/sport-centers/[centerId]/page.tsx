...
export async function generateStaticParams() {
  try {
    const response = await fetchAndCatch(`${API_URL}/sportcenter/search`, { method: "GET" });

    const { sport_centers }: { sport_centers: { id: string }[] } = await response;
...
const SportCenterPage = async ({ params }: { params: Promise<{ centerId: string }> }) => {
  try {
    ...
    const sport_center: ISportCenter = await fetchAndCatch(`${API_URL}/sportcenter/${id}`,
      { cache: "no-store" }
    );
    ...
    if (error instanceof ErrorHelper) {
      ...
      if (error.message === ApiStatusEnum.CENTER_NOT_FOUND) {
        ...
        return (
          <div>No se encontraron canchas para este centro deportivo</div>
        );
