  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {users.length === 0 ? (
      ...
      users.map((user) => (
        <div
          key={user.id}
          className="bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 text-white p-6 rounded-lg shadow-lg flex flex-col items-center hover:scale-105 transition-transform duration-300 ease-in-out"
        >
          {/* Imagen de perfil */}
          ...
          <p className="text-sm mt-2 text-center">Usuario Premium</p>
        </div>
      ))
    )}
  </div>
